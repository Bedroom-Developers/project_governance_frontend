...
- **Соблюдение правил качества**: Biome (линт и форматирование), импорт-ордера, JSDoc для публичных API.
...
- Biome обязателен (линт и форматирование), не допускаются неиспользуемые импорты/переменные.
...
- **Biome для shared:** сортировка импортов (organizeImports) отключена для `src/shared/**` в `biome.json`.
...
В проекте настроен **Husky** для автоматических проверок при коммите и пуше.

- **Pre-commit** (`git commit`):
  - **lint-staged** — Biome проверяет и правит только файлы в коммите (линт, формат, импорты).
  - **Unit-тесты** — `pnpm run test:run`.

Конфигурация: `.husky/` в корне проекта, секция `lint-staged` в `package.json`. После `pnpm install` хуки подхватываются автоматически (скрипт `prepare`).
